  ...
  let server = createServer({
    ...
    routes() {
      ...
      this.post('/vet/pets', (schema, request) => {
        ...
        if (gyoker === 400) {
          return new Response(gyoker, {status: isVaccinated}, {status: name + " nincs"})
        } else {
          return new Response(gyoker, { status: isVaccinated }, { status: isVaccinated ? name + " él" : name + " meghal"});
        }
      });
